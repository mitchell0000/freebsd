...
#if defined(_LIBUNWIND_TARGET_RISCV32)
/// Registers_riscv32 holds the register state of a thread in a 32-bit RISC-V
/// process.
class _LIBUNWIND_HIDDEN Registers_riscv32 {
public:
  Registers_riscv32();
  Registers_riscv32(const void *registers);
  ...
  uint32_t  getSP() const         { return _registers[2]; }
  void      setSP(uint32_t value) { _registers[2] = value; }
  uint32_t  getIP() const         { return _registers[1]; }
  void      setIP(uint32_t value) { _registers[1] = value; }
...
inline Registers_riscv32::Registers_riscv32(const void *registers) {
  static_assert((check_fit<Registers_riscv32, unw_context_t>::does_fit),
                "riscv registers do not fit into unw_context_t");
  memcpy(&_registers, registers, sizeof(_registers));
...
inline Registers_riscv32::Registers_riscv32() {
  memset(&_registers, 0, sizeof(_registers));
  memset(&_floats, 0, sizeof(_floats));
}

inline bool Registers_riscv32::validRegister(int regNum) const {
  if (regNum == UNW_REG_IP)
    return true;
  if (regNum == UNW_REG_SP)
    return true;
...
inline uint32_t Registers_riscv32::getRegister(int regNum) const {
  if (regNum == UNW_REG_IP)
    return _registers[1];
  if (regNum == UNW_REG_SP)
    return _registers[2];
...
inline void Registers_riscv32::setRegister(int regNum, uint32_t value) {
  if (regNum == UNW_REG_IP)
    _registers[1] = value;
  else if (regNum == UNW_REG_SP)
    _registers[2] = value;
...
inline const char *Registers_riscv32::getRegisterName(int regNum) {
  switch (regNum) {
  case UNW_REG_IP:
    return "pc";
  case UNW_REG_SP:
...
inline bool Registers_riscv32::validFloatRegister(int regNum) const {
  if (regNum < UNW_RISCV_F0)
    return false;
  if (regNum > UNW_RISCV_F31)
    return false;
...
inline double Registers_riscv32::getFloatRegister(int regNum) const {
#if defined(__riscv_flen) && __riscv_flen == 64
  assert(validFloatRegister(regNum));
  return _floats[regNum - UNW_RISCV_F0];
#else
...
inline void Registers_riscv32::setFloatRegister(int regNum, double value) {
#if defined(__riscv_flen) && __riscv_flen == 64
  assert(validFloatRegister(regNum));
  _floats[regNum - UNW_RISCV_F0] = value;
#else
...
inline bool Registers_riscv32::validVectorRegister(int) const {
  return false;
}

inline v128 Registers_riscv32::getVectorRegister(int) const {
  _LIBUNWIND_ABORT("no riscv vector register support yet");
}

inline void Registers_riscv32::setVectorRegister(int, v128) {
  _LIBUNWIND_ABORT("no riscv vector register support yet");
}
#endif // _LIBUNWIND_TARGET_RISCV32
